...
function normalizeProduct(input) {
  ...
  return {
    ...
    aesthetic_tags: toArray(input.aestheticTags || input.aesthetic_tags),
    room_type_tags: toArray(input.roomTypeTags || input.room_type_tags),
    color_tags: toArray(input.colorTags || input.color_tags),
    shipping_regions: toArray(input.shippingRegions || input.shipping_regions || "Worldwide"),
    room_glow_up_enabled: input.roomGlowUpEnabled ?? input.room_glow_up_enabled ?? true,
    availability: input.availability || "active",
    affiliate_network: input.affiliateNetwork || input.affiliate_network || input.store || "Amazon",
    editorial_priority: typeof input.editorialPriority === "number" ? Math.max(0, Math.min(10, input.editorialPriority)) : 0,
